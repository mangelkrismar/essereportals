...
<header class="p_header">
		...
		<div class="p_headertransition">    
			<? 
				if($qval==2){
					...
					echo '<div class = "p_headerImg5" ></div>';
					echo '<div class = "p_headerImg4"></div>';
			...
			<div class="p_logonova <?=($this->bandera_movil)?"p_logonovasmall":""?>" >
				...
				<script type="text/javascript">
					$( document ).ready(function() {
						...
						$(".p_ingresarsalir").on('click', function() {
							sessionStorage.removeItem('cenFlag'+tabID);
						});

						var cenFlag = "<?php echo $cenFlag ?>";
						if (!sessionStorage.getItem("cenFlag"+tabID) || cenFlag=="true") {
							sessionStorage.setItem('cenFlag'+tabID, cenFlag=="true");
						}
					    if(""+sessionStorage.getItem("cenFlag"+tabID)=="true"){
					    	$('#logoPage').css({"background-image":"url('src/img/logoCEN.png')","width":"100%","height":"100%","background-size":"contain","background-repeat":"no-repeat"});
					    }
					});
				</script>
			</div>
	...
	<nav class="p_navsup" style="min-height: 26px;">  
