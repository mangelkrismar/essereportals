...
app.controller("CursoController", function($scope, $http, $window){
	...
	$scope.clasePTema = function(nombre){
		...
		if(nombre){
			switch(nombre.toLowerCase()){
				...
				case 'habilidades digitales'   : return 'p_temas_inf';
	...
	$scope.enviaGClass = function(flag,app_id){
		//Configurar la infomación de GA4 - Col
		$scope.sendDataGA4_Col(app_id);
		/*
	...
	$scope.showHideElement = function(idElement, indi, ...app){
		//Configurar la información de GA4 - Col
		$scope.sendDataGA4_Col({ i : app[0], p : app[1], n : app[2] });
	...
	/*					    */
	/* GOOGLE ANALYTICS COL */
	/**
	 * Configura los parametros e enviar al evento GTM
	 * @param {object} app Contiene la información de la app
	 */
	 $scope.sendDataGA4_Col = function(app){
		//Variables de configuración de metadata
		let GA4_TAG    = 'G-0698KRMXTJ';
		let GA4_PORTAL = 'Secundaria';
		//Estalecer la metadata del evento
		window.dataLayer = window.dataLayer || [];
		function gtag(){dataLayer.push({
			'app_id'     : app.i,
			'app_prefix' : app.p,
			'app_name'   : app.n,
			'portal_name': GA4_PORTAL
		});}
		gtag('js', new Date());
		gtag('config', GA4_TAG);
	}
