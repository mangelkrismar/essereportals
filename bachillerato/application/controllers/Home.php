...
class Home extends MY_Controller{	
	...
	public function index(){
		...
		if($this -> session ->userdata('log_in')){//Esta logeado
			$datos['apps_reciente'] = json_decode(file_get_contents("src/json/apps_recienteBachillerato2023.json"));
			$this->load->view('body/body_user', $datos);
		}else{
			//Traemos aplicaciones de "conoce nuestras aplicaciones"
			$datos['apps_conoce'] = json_decode(file_get_contents("src/json/apps_conoceBachillerato2023.json"));
	...
	public function keepEmail(){
		...
		$actual .= $email." -|- bachillerato -|- ".date('l jS \of F Y h:i:s A')."\n";
