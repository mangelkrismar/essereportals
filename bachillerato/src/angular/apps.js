...
*/
/*    GOOGLE CLASSROOM     */
var aplicationID = null; //Variable de ID  de Classroom
var catego;
/*                         */
...
app.controller("CursoController", function($scope, $http, $window){
	...
	$scope.clasePTema = function(nombre){
		...
		if(nombre){
			switch(nombre.toLowerCase()){
				case 's1': case '1°': case 'primer grado' : return 'p_temas_primero';
				case 's2': case '2°': case 'segundo grado': return 'p_temas_segundo';
				case 's3': case '3°': case 'tercer grado' : return 'p_temas_tercero';
				case 's4': case '4°': case 'cuarto grado' : return 'p_temas_cuarto';
				case 's5': case '5°': case 'quinto grado' : return 'p_temas_quinto';
				case 's6': case '6°': case 'sexto grado'  : return 'p_temas_sexto';
				
				
				case 'artes'                   		: return 'p_temas_art';
				case 'atlas'                   		: return 'p_temas_atl';
				case 'biología'                		: return 'p_temas_bio';
				case 'ciencias'                		: return 'p_temas_nat';
				case 'ciencias de la salud'    		: return 'p_temas_csa';
				case 'ciencias naturales'      		: return 'p_temas_nat';
				case 'ciencias sociales'       		: return 'p_temas_cso';
				case 'ciencias de la comunicación': return 'p_temas_esp';
				case 'derecho'                 		: return 'p_temas_der';
				case 'desarrollo comunitario'  		: return 'p_temas_dco';
				case 'ecología'                		: return 'p_temas_eco';
				case 'educación artística'     		: return 'p_temas_art';
				case 'ética y valores'     				: return 'p_temas_cye';
				case 'español'                 		: return 'p_temas_esp';
				case 'estructura socioeconómica de méxico'	: return 'p_temas_esm';
				case 'filosofía'               		: return 'p_temas_fil';
				...
				case 'historia de méxico'      		: return 'p_temas_his';
				case 'historia universal contemporánea'	: return 'p_temas_hisu';
				case 'informática'             		: return 'p_temas_inf';
				case 'inglés'                  		: return 'p_temas_ing';
				case 'lecturas' 							 		: return 'p_temas_lec';
				case 'lecturas de comprensión' 		: return 'p_temas_lec';
				case 'lenguaje y comunicación' 		: return 'p_temas_esp';
				case 'literatura'              		: return 'p_temas_lit';
				case 'matemáticas'             		: return 'p_temas_mat';
				case 'metodología de la investigación' : return 'p_temas_met';
				case 'ofimática'             	 		: return 'p_temas_inf';
				case 'probabilidad y estadística' : return 'p_temas_pro';
				case 'química'                 		: return 'p_temas_qui';
				case 'taller de lectura y redacción'   : return 'p_temas_tle';
				case 'tecnología'              		: return 'p_temas_inf';
			}
			return '';
		}
		return '';
		...
		if( $scope.catAplicacion ||
			...
			switch (true) {
				...
				case /simulador/gi.test(aplicacion.c):
					if(!(/^red_his_8.*/.test(aplicacion.p))){
						if($scope.catSimulador) return true; break;
					}
	...
	$scope.abrirApp= function(aplicacion, windowReference){
		...
		if(windowReference!=null){
			windowReference.location = $scope.kapps_url + "index.php/recurso/cargarApp/"+aplicacion.i+"/"+empresa;
		}else{
			window.open($scope.kapps_url + "index.php/recurso/cargarApp/"+aplicacion.i+"/"+empresa);
		}
	}
	...
	$scope.getAppLink = function(id_aplicacion){
		...
		*/
		return $scope.kapps_url + "index.php/recurso/cargarApp/"+id_aplicacion+"/bachillerato";
		
	}

	/*$scope.shareToGClass = function(app){
		console.log(app);
		let dt = new Date();
		let generatedID = window.btoa(dt.getDate()+''+(dt.getMonth()+1)+''+dt.getFullYear()+''+dt.getHours() + "" + dt.getMinutes() + "" + dt.getSeconds()+''+app.p);
		var link = $scope.kapps_url+'index.php/recurso/cargarApp/'+app.i+'/primaria?id='+generatedID;
		window.open("https://classroom.google.com/share?url=" + encodeURIComponent(link)+"&title="+encodeURIComponent(app.n));
	}

	$scope.shareToGClass2 = function(app_id, app_n, app_p){
		let dt = new Date();
		let generatedID = window.btoa(dt.getDate()+''+(dt.getMonth()+1)+''+dt.getFullYear()+''+dt.getHours() + "" + dt.getMinutes() + "" + dt.getSeconds()+''+app_p);
		var link = $scope.kapps_url+'index.php/recurso/cargarApp/'+app_id+'/primaria?id='+generatedID;
		window.open("https://classroom.google.com/share?url=" + encodeURIComponent(link)+"&title="+encodeURIComponent(app_n));
	}*/
		...
		*/
		...
		switch(categoria){
			...
			case 'lectura':
				...
				break;
			case 'simulador':
				$scope.catSimulador = !$scope.catSimulador;
				break;
		}
		$scope.resetPagination();
	}
