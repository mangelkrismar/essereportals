...
$(document).ready(function(){
    ...
    /*PARA PDF SECUNDARIA */
    secusername = localStorage.getItem('secusername'); //Obtener el nombre que se encuentra en el localStorage
    var json = { //Constriur un JSON con el dominio origine y el nombre de usuario
        'origin': IP,
        'secusername': secusername
    };
    window.parent.postMessage(JSON.stringify(json), '*'); //Envíar al iframe padre
});
...
function muestraComprar(mensaje){
    ...
    */
	...
	//para saber que url val
	switch (curl) {
		case 'mx':
			qurl = 'muestraSecMx.php';
			break;
		case 'lat':
			qurl = 'muestraSecLat.php';
			break;
		default:
			qurl = 'muestraSecMx.php';
			break;
		...
		success: function(data){
			...
			switch(String(json.status)){
				...
				case '1':
					...
					if(curl != 'mx'){	
						document.getElementById("cProducto").innerHTML  = "";
						document.getElementById("cProducto").innerHTML = json.preciov;
					}	 					
...
/*******PAARA LO DEL CAMBIO DE USUARIO********** */
...
function eligeOPC(opc){
	...
	var moneda = $('option:selected', opc).attr("moneda");
	document.getElementById("cProducto").innerHTML  = "$ "+saldo+" "+moneda;
	
	$(".p_emeralerta").text("");
	//$(".p_emeralerta").text($("#gradoCompra").val());  //gradoCompra
}

/********VALIDAR CAMPOS CAMBIA DE USUARIO************** */
    ...
    */
      if(valor.acces){
        if(valor.cambiarUsuario){
            ...
            /*setTimeout(function(){
                $(".p_emerboxconte").hide();
                $(".p_emerbox").removeClass("bounceOut");
                $(".p_emerboxbienvenido").show();
                $(".p_emerboxbienvenido").parent().addClass("bounceIn");
                
                setTimeout(function(){
                    $(".p_emergenteusuario").fadeOut(3000);
            ...
            /* PARA PDF SECUNDARIA */
           localStorage.setItem('secusername', valor.secusername); //Guardar el nombre de usuario en local storage
    }else{
