...
function regresarLibros(){
	...
	$("#iconregresarlibro, #libros_sep, #clasicos").hide();
	...
	* VARIABLES	*/
	...
	if(menuVisible){
		...
		$(".p_menusup").stop().animate({"top":(altoHeader-$(".p_menusup").height())+"px","opacity":"1"}, 500, function(){
			$(".contenido, .maskrismar, .britannica, .libros").hide();
			$("."+ideSubtema).show();
	...
	$("#contenido, #maskrismar, #britannica, #libros, #contacto").removeClass("p_navsupinbtn_indicador");
		...
		if($(".p_headerin").height() >= $(document).scrollTop()){
			$(".p_flotanteicon").stop().fadeOut(250);
             ...
             if($(this).scrollTop() > ultimoScroll){
                ...
                $(".p_headerImg1, .p_headerImg2, .p_headerImg3, .p_headerImg4, .p_headerImg5").css({"background-position":"0% "+(70-((350+$(this).scrollTop())/10))+"%"});
...
function headerTransit(){
	...
	*/
	...
	if($(document).scrollTop() >= 170 ){
		...
		$("#sombras").show();
        $(".p_header, .p_headerdatos, .p_headersmall, .p_section, .p_menusup, #buscar, #divForApps").removeAttr("style");//El header aparece con imagen y es parte del scroll.
        if(bandConf){
...
function ocultaBanner(){
	...
	*/
	...
	if($("#is_movil").length 	 == 1){//es pc
		...
		$("#buscar").css({"position":"fixed","top":58,"z-index":1,"background-color":"#F2F2F2", "background-image":"url(src/img/p_bodynext.png)","background-position":"center","background-size":"cover","background-attachment":"fixed","background-repeat":"repeat","margin-top":"18px"});
		$("#divForApps").css({"margin-top":150});
	}else{//es movil
		//170 PORQUE LA MEDIDA DEL HEADER ES FIJA DE 220 +26 DEL MENU AZUL(LAS MEDIDAS SIEMPRE SON FIJAS)
		$(".p_section").css({"margin-top":$(".p_header").height()+"px"});//La sección se posiciona con un margen.
		$("#buscar").css({"position":"fixed","top":110,"z-index":1,"background-color":"#F2F2F2", "background-image":"url(src/img/p_bodynext.png)","background-position":"center","background-size":"cover","background-attachment":"fixed","background-repeat":"repeat","margin-top":"18px"});
		$("#buscar").before('<div id="buscarPanel"></div>');
		$("#buscarPanel").css({"margin":0,"top":0,"width":"100%","height":150,"position":"fixed","z-index":1,"background-color":"#F2F2F2", "background-image":"url(src/img/p_bodynext.png)","background-position":"center","background-size":"cover","background-attachment":"fixed","background-repeat":"repeat"});
		$("#divForApps").css({"margin-top":150});
	}
...
function execPlayDemo(numero, nombre, lugar, windowReference){
	...
	* VARIABLES:
	*/
	window.open(IPSRC + "recurso/cargarApp/"+numero+"/primaria");
}
...
function checkNet(){
	...
	req.open('POST', 'home/netAccess', true);
	req.onreadystatechange = function (aEvt) {
	  	if (req.readyState == 4) {
			if(req.status == 200){
				online = (req.responseText==0);
			}else{
				online=false;
			}
	  	}
...
$(document).ready(function(){
	$('.mustBeLogged').click(function(event) {
        ...
        var a = $(this);
        var url = null;
        if(a.attr('class').includes('Efemerides')){ url = IP+"../Efemerides"; window.open(url); }else
        if(online){
        	if(a.attr('class').includes('Krismar')){ url = "http://www.krismar.com.mx"; }
        	if(a.attr('class').includes('Dir')){ url = "https://www.google.com.mx/maps/place/Krismar+Computaci%C3%B3n+Toluca/@19.275963,-99.5790527,17z/data=!3m1!4b1!4m5!3m4!1s0x85cd8a1dbca94065:0x8f322b025b3e280c!8m2!3d19.275963!4d-99.576864"; }
        	if(a.attr('class').includes('Fb')){ url = "https://www.facebook.com/Krismar-Educaci%C3%B3n-547605475386249/"; }
        	if(a.attr('class').includes('Yt')){ url = "https://www.youtube.com/user/krismareduca"; }
        	if(a.attr('class').includes('Tr')){ url = "https://twitter.com/KrismarEduca?lang=es"; }
	        if(a.attr('class').includes('Escolar')){ url = "http://www.krismar-educa.com.mx/primaria/primaria.php"; }
			if(a.attr('class').includes('School')){ url = "http://www.krismar-educa.com.mx/primaria/escolar.php"; }
			if(a.attr('class').includes('Biografía')){ url = "http://www.krismar-educa.com.mx/primaria/biografias.php"; }
			if(a.attr('class').includes('Mapas')){ url = "http://www.krismar-educa.com.mx/primaria/mapas.php"; }
			window.open(url);
        }else{
			muestraNetBlock();
        }
