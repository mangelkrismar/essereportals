...
function cargaBodyListo(){
	...
	$.get("src/xml/objetos16.xml", function(xml){
		
		$(xml).find("datos").each(function(index, item){
			
			//console.log($(item))
			
			$("[bole = '"+$(item).attr("bole")+"']").parent().append("<a style = 'display:none'  target = '_blank' href = '"+$(item).attr("rutaD")+"'><ul>"+$(item).attr("textoD")+"</ul></a>");
			$("#lista2").find("a").css({"display": "block"});
				
			
			//$("#lista2").append('<ul>'+$(item).attr("textoD")+'</ul>');	
		});

		$("[bole]").each(function(index,item){
			$(this).parent().off().click(function(event){
				toggleLibros($(this).children()[0]);
			});
		});
...
function regresarLibros(){
	...
	$("#iconregresarlibro, #libros_sep2, #libros_sep, #clasicos, #museosLat, #GRcontainer").hide();
...
function muestraLibrosSecundaria(){
	$("#libros_sep2, #libros_sep, #clasicos, #museosLat, #GRcontainer").hide();
	$("#lista2").removeClass("catalogosBackground");
	$("#lista2").addClass("catalogosBackground");
	$("#libros_sep").find(".p_articletitle").text("Libros de secundaria");
	$.post(
		IP+"Libros2/secundaria",
		function(data){
			var result = JSON.parse(data);
			$("#lista2").empty();
			$.each(result, function(grado, libros){
...
function muestraLibrosGuiaProf(){
 ...
 function muestraLibrosAlumno(){
	$("#libros_sep2, #libros_sep, #clasicos, #museosLat, #GRcontainer").hide();
	$("#libros_sep2").find(".p_articletitle").text("Libros SEP");
	$.post(
		IP+"Libros2/libros",
		function(data){
			var result = JSON.parse(data);
			...
			if($("#libros_sep2").css("display") == "none"){
				$("#libros_sep2").css("display","table");
			}
			$("body").stop().animate(
				{
					scrollTop: $("#libros_sep2").offset().top - (altoNavegacion + 20)
				},
				500,
				"linear",
				function(){
...
function muestraLibrosCatalogo(){
	$("#libros_sep2, #libros_sep, #clasicos, #museosLat, #GRcontainer").hide();
	$("#libros_sep2").find(".p_articletitle").text("Libros SEP");
	$.post(
		IP+"Libros2/catalogos",
		function(data){
			var result = JSON.parse(data);
			$("#listaN").empty();
			$.each(result, function(grado, libros){
...
function toggleLibros(obj){
	...
	$(obj).parent().find("a").toggle("slow",function(){
	});
}
...
function obtenDisp(){
	...
	*/
	...
	if($("#device").attr("value") == "movil"){muestra
		disp = "movil";
		ocultaBanner();
		resizeMovil();
		colocaEmergente();
...
function muestraLinksAdicionales(){
...
function muestraLinksAdicionalesLat(){
	
	
	$("#listaLat").find("a").hide();
	$("#libros_sep2, #libros_sep, #museosLat, #clasicos, #GRcontainer").hide();
	
	if($("#museosLat").css("display") == "none"){
		$("#museosLat").css("display","table");
		$("body").stop().animate({scrollTop: $("#museosLat").offset().top - (altoNavegacion + 20)},500, "linear", function(){
			ocultaMenuAnimado();
			$("#iconregresarlibro").show();
