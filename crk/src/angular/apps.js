...
app.controller("CursoController", function($scope, $http, $window){
	...
	$scope.filtrarCategorias = function(aplicacion){
		...
		*/
		if( $scope.catAplicacion ||
			...
			$scope.catEvaluacion){
			switch(true){
                case /aplicacion.*/.test(aplicacion.c):
					if($scope.catAplicacion) return true; break;
                case /simulador.*/.test(aplicacion.c):
					if($scope.catEvaluacion) return true; break;
				case /lectura/.test(aplicacion.c):
					if($scope.catLectura) return true; break;
				case /video.*/.test(aplicacion.c):
	...
	$scope.abrirApp= function(aplicacion, windowReference){
		 ...
		 */
        if(!$scope.logged_in){ muestraIngresar(); return true; }
		if($scope.ccdig){
	...
	$scope.getAppLink = function(id_aplicacion){
		...
		*/
		return $scope.kapps_url + "index.php/recurso/cargarApp/"+id_aplicacion+"/primaria";
    ...
    $scope.loginrequest = function(){
        ...
        request.done(function(response, textStatus, jqXHR){
            ...
            $scope.compruebaAcceso(responseJSON);
        });
    ...
    $scope.compruebaAcceso = function(valor){
        $scope.logged_in = valor.acces;
        $scope.$apply();
        compruebaAcceso(valor);
    }
});